...
static int _udpbd_read(struct block_device* bd, u32 sector, void* buffer, u16 count)
{
	...
	M_DEBUG("%s: sector=%u, count=%d ERROR=%u\n", __func__, sector, count, g_errno);
    g_errno = 0;
    debug_smap();
...
static int udpbd_read(struct block_device* bd, u32 sector, void* buffer, u16 count)
{
	...
	while (count_left > 0) {
        ...
        sector += count_block;
        buffer = (void *)((u8 *)buffer + (count_block * g_udpbd.sectorSize));
	}
...
void udpbd_rx(u16 pointer)
{
	USE_SMAP_REGS;
    union {
        udpbd_header_t udpbd_header_struct;
        u32 arr[4];
    } hdr_storage;
	udpbd_header_t *hdr = &hdr_storage.udpbd_header_struct;

    hdr_storage.arr[0] = UDPBD_HEADER_MAGIC;
	hdr_storage.arr[1] = SMAP_REG32(SMAP_R_RXFIFO_DATA); // CMD
	hdr_storage.arr[2] = SMAP_REG32(SMAP_R_RXFIFO_DATA); // PAR1
	hdr_storage.arr[3] = SMAP_REG32(SMAP_R_RXFIFO_DATA); // PAR2
