...
static void ieee1394Swab32(void *dest, void *src, unsigned int nQuads){
	...
	for(i=0; i<nQuads; i++) ((unsigned int *)dest)[i]=BSWAP32(((unsigned int *)src)[i]);
}
...
int iLinkGetNodeCapabilities(unsigned short NodeID){
	...
	if(NodeID==LocalNodeID) return NodeCapabilities;
...
static inline int ParseUnitCROM(unsigned short NodeID, unsigned int *UnitSpec, unsigned int *UnitSW_Version){
	...
	if((result=iLinkReadCROM(NodeID, CROMOffset, 1, &data))<0) return(result);
...
void InitializeConfigurationROM(void){
	...
	for(i=0; i<16; i++) ExtraCROMUnits[i]=NULL;
	nExtraCROMUnits=0;
...
static void BuildConfigurationROM(void){
	...
	for(i=0; i<16; i++) if(ExtraCROMUnits[i]!=NULL) TotalExtraCROMUnitSize+=ExtraCROMUnitsSize[i];
	...
	if((CROM_Buffer=malloc(CROMSize))==NULL) return;
	...
	if(ConfigurationROM!=NULL) free(ConfigurationROM);
