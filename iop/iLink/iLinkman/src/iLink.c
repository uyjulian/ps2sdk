...
int _start(int argc, char **argv){
	...
	for(i=0; i<MAX_CONCURRENT_TRANSACTIONS; i++) TransactionContexts[i].IsConnected=0;
...
unsigned int iLinkSetNodeCapabilities(unsigned int capabilities){
	...
	if(NodeCapabilities&iLink_NODE_CMC) iLinkEnableCMaster();
